...
function AgentAccess({ status }: { status: YoutubeStatus }) {
  ...
  const instructions = `Connect to my AgentWay publishing bridge at ${base}. Authenticate with a bearer token supplied through your secure credential prompt. Never ask me to paste Google credentials into chat. Read GET /v1/status. To transfer a finished video, POST /v1/media with {"size":<exact bytes>,"mime":"video/mp4"}, then PUT raw bytes to the returned upload_path using the same bearer token. POST /v1/youtube/publish with {"request_id":<new UUID>,"media_id":<returned ID>,"title":<title>,"description":<description>,"privacy":"private","made_for_kids":false,"contains_synthetic_media":<appropriate boolean>}. Reuse the same request_id and identical arguments on retries. Poll GET /v1/publications/{id} every five seconds while queued or uploading. Stop and report an interrupted upload; POST /v1/publications/{id}/retry resumes the same session. Only report success when video_url is returned. Upload completion does not mean YouTube has finished processing or classified it as a Short. MCP clients can use ${base}/mcp with the same Authorization: Bearer token.`;
  return (
    <section
      className="panel publishing-section"
