...
- Current phase: **0 — common receiver contract and feasibility**. The [receiver compatibility matrix](handoff-receiver-compatibility.md) records implementation risks, not the design. No live native receiver test has passed; no receiver binding, dispatcher, callback, or return-delivery code exists. Next action: freeze one AgentWay-facing receiver protocol, state machine and cross-product conformance suite; then map each native product to that contract and run harmless live proofs. Do not design the system around Grok or Codex's routine feature.
