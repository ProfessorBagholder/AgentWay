...
Not runnable end to end yet. `/v1/agent-tasks` is a pull inbox; a sender receipt now records result readback separately from completion, but no verified binding, native wake adapter or idle-sender return path exists. The Grok Bot → Codex task `80774cfa-1d40-444d-bd95-445ba7633d51` required owner ID relay. The later task `8387e338-6d2b-4272-b1e6-2924eaaaca78` was picked up by this same Codex conversation through a scheduled heartbeat, without ID relay; Grok Bot was active and polling for its result. In the reverse test, task `277785fe-44a6-48b7-851f-2ba5f6d67164` remained queued because Grok Bot's saved routine reported `never run`; Grok completed it only after owner prompting, and Codex then recorded a sender receipt. That reverse test **failed unattended pickup**. None of these runs passes the idle bidirectional round trip or full suite. A synthetic queue/claim test cannot substitute for native evidence.

After the owner corrected Grok's routine setup, a new reverse task `ce69ec4d-0646-4df7-88fc-2a03a0adc65a` was claimed by Grok ~4m55s after creation and completed with the expected result. The existing Grok conversation showed the matching task ID and the routine pausing itself; Codex then recorded a sender acknowledgment while already active. This passes the scheduled Grok pickup slice only. It does not erase the failed first run or pass idle sender return and the rest of this suite.

The subsequent idle-to-idle task `d9b6ecc0-ace6-4579-8c93-3078e2f4a648` received no Grok claim after more than two five-minute intervals. The sender cancelled it and stopped its checks; the Grok routine was then paused in its native UI and the Resume control verified. This is a failed run. Every future probe must set a finite deadline before creation (at most two receiver intervals plus a stated grace period for this candidate), then automatically stop polling, cancel or retain the task under an explicit recovery policy, revoke temporary grants and report a failure. An indefinite silent `queued` loop is not conformance.
