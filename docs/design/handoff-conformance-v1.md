...
Not runnable end to end yet. The existing `/v1/agent-tasks` API is a pull inbox and no verified receiver binding, native wake adapter or result-return path exists. The Grok Bot → Codex task `80774cfa-1d40-444d-bd95-445ba7633d51` required the owner to relay the ID, so it fails the idle round-trip case. The next live probe must first establish a supported native ingress and return path for an existing bound agent; a synthetic AgentWay queue/claim test cannot substitute for it.
